...
- **[`coco-chatbot/`](context-test-docs/n8n-workflows/coco-chatbot/)** — agentes conversacionales Coco y Nora
  - `ChatBot.json` — recepción de mensajes de ManyChat (WhatsApp), normalización de teléfono, blacklist, buffer/dedup de mensajes en Redis, UPSERT de `clients`, **enrutamiento por `is_vip`** hacia Coco o Nora (nodo `IF: Es VIP`), detección de la marca `[[PAGO_VALIDADO]]` y disparo del webhook `Save VIP User`
...
Workflows activos en n8n (verificado vía API el 2026-07-04): `ChatBot`, `Encuesta: Obtener configuración`, `Save Responses`, `Save VIP User`, `Send Messages` (tenía 4 bugs de referencias/config + un bug de conexión rota (`Split Out → Loop Over Items`, perdía cualquier respuesta de 2+ mensajes en silencio), todos corregidos y verificados el 2026-07-04 — ver `WEBHOOKS.md` §6 y §8), `ManyChat - Toggle BotStatus` (creado por el usuario). `Coco Agent` (el dev/test) está inactivo.
