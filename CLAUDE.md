...
- **`evaluacion/`** — `Save-Responses.json` (webhook del quiz: valida contra la config, UPSERT de `clients`, INSERT de `quiz_submissions`, llama a `Send Messages`) y `Encuesta-Obtener-configuracion.json` (devuelve la estructura de preguntas/opciones desde Postgres).
